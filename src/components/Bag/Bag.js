...
function Bag() {
  ...
  useEffect(() => {
    setTotalMrp(
      bagList.reduce(
        (acc, item) => acc + Number(item.strickPrice * item.quantity),
        0
      )
    );
    setDiscount(
      bagList.reduce(
        (acc, item) =>
          acc +
          Number(item.strickPrice * item.quantity) -
  ...
  const checkoutHandler = async () => {
    ...
    const {
      ...
      "https://myntraa-backend-2.onrender.com/api/checkout",
      { totalAmount }
    );
    const {
      data: { key },
    } = await axios.get("https://myntraa-backend-2.onrender.com/api/getkey");
    ...
    const options = {
      ...
      order_id: order.id,
      callback_url:
        "https://myntraa-backend-2.onrender.com/api/paymentverification",
      prefill: {
        name: "Abdul",
  ...
  return (
    <>
      ...
      {bagList.length > 0 ? (
        <div className="bag-main">
          <div>
            <div className="address">
              <div>
                <p>
                  Deliver to: <span>Abdul Aziz,742140</span>
                </p>
            ...
            <div className="bag-offers">
              ...
              <ul>
                <li>
                  10% Instant Discount on IndusInd Bank Debit Cards on a min
                  ...
                  <div style={{ display: "flex" }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontWeight: "bold" }}>
                        Qty:
                        <button
                          onClick={() => handelQtyDec(item.id, item.quantity)}
                        >
                          -
                        </button>
                        {item.quantity}
                        <button
                          onClick={() => handelQtyInc(item.id, item.quantity)}
                        >
                          +
                        </button>
                      </p>
                      <div className="bag-prices">
                        ...
                        <p style={{ textDecoration: "line-through" }}>
                          ₹{item.strickPrice * item.quantity}
                        </p>
        ...
        <div className="bag-empty">
          <img
            ...
            alt="emptyBag"
          />
          <p style={{ fontWeight: "bold", marginBottom: 0, fontSize: 20 }}>
            Hey, it feels so light!
          </p>
          <p style={{ marginTop: 1, fontSize: 15 }}>
      ...
      <div className="bag-footer">
        ...
        <div>
          <a target="__blank" href="https://www.linkedin.com/in/aziz7477/">
